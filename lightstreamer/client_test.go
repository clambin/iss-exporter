...
func TestClient(t *testing.T) {
	...
	ts := httptest.NewServer(NewServer("mySet", "myCID", l))
	t.Cleanup(ts.Close)
...
func TestClient_ISS(t *testing.T) {
	...
	if err = conn.Subscribe(ctx, "DEFAULT", "TIME_000001", []string{"Value"}, func(item int, values []string, l *slog.Logger) {
	...
	if err = conn.Subscribe(ctx, "DEFAULT", "NODE3000011", []string{"Value"}, func(item int, values []string, l *slog.Logger) {
